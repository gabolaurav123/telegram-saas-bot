...
from app.keyboards.user import cancel_purchase_keyboard, main_menu_keyboard
...
async def cb_select_payment_method(
    ...
    if plan is None or method is None or not method.is_active:
    ...
    await state.update_data(plan_id=plan.id, payment_method_id=method.id)
